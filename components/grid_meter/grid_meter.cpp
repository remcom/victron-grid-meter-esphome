...
void GridMeterComponent::write_int32_(uint16_t *regs, uint8_t idx, int32_t val) {
  regs[idx]     = (uint16_t)((uint32_t)val >> 16);
  regs[idx + 1] = (uint16_t)((uint32_t)val & 0xFFFF);
}

void GridMeterComponent::write_uint32_(uint16_t *regs, uint8_t idx, uint32_t val) {
  regs[idx]     = (uint16_t)(val >> 16);
  regs[idx + 1] = (uint16_t)(val & 0xFFFF);
}
...
void GridMeterComponent::setup() {
  ...
  ESP_LOGI(TAG, "Grid meter Modbus TCP server listening on port 502");
}
...
void GridMeterComponent::refresh_sensors_() {
  // Voltage (int32, x0.1 V) -- hold last good on NaN
  float v = voltage_->get_state();
  if (!std::isnan(v)) {
    int32_t v_raw = (int32_t)(v * 10.0f + 0.5f);
    voltage_shadow_[0] = (uint16_t)((uint32_t)v_raw >> 16);
    voltage_shadow_[1] = (uint16_t)((uint32_t)v_raw & 0xFFFF);
  }
  registers_[0] = voltage_shadow_[0];
  registers_[1] = voltage_shadow_[1];

  // Current (int32, x0.001 A) -- hold last good on NaN, always positive magnitude
  float i = current_->get_state();
  if (!std::isnan(i)) {
    int32_t i_raw = (int32_t)(std::abs(i) * 1000.0f + 0.5f);
    current_shadow_[0] = (uint16_t)((uint32_t)i_raw >> 16);
    current_shadow_[1] = (uint16_t)((uint32_t)i_raw & 0xFFFF);
  }
  registers_[2] = current_shadow_[0];
  registers_[3] = current_shadow_[1];

  // Net power (int32, x0.1 W) -- positive = import, negative = export; zero on NaN
  ...
  if (!std::isnan(imp) && !std::isnan(exp_pwr)) {
    ...
    write_int32_(registers_, 4, pwr_raw);
  } else {
...
void GridMeterComponent::handle_frame_(Client &c, uint16_t frame_len) {
  ...
  if (fc == 0x03 || fc == 0x04) {
    // FC03 / FC04: Read Holding/Input Registers (both return identical responses;
    // the Victron Cerbo GX may use either function code depending on firmware version)
...
void GridMeterComponent::send_response_(int fd, uint16_t txid, uint8_t uid,
  ...
  // Fixed-size: max pdu_len = 2 + REG_COUNT*2 = 38, so max frame = 45 bytes
  uint8_t frame[7 + 2 + REG_COUNT * 2];
